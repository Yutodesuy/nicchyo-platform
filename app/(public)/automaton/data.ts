...
したがって、M' は A/B を受理する。よって A/B は正規言語である。`,
  ...
  {
    id: "5.2",
    question: "任意の文脈自由言語 L に対して,下記の条件を満たす非負整数 k ∈ Z≥0 が存在することを証明せよ:長さが少なくとも k である任意の文字列 s ∈ L に対して,次の三つの条件を満たす 5 個の断片 s = uvxyz に分割できる: (i) 全ての非負整数 i ∈ Z≥0 に対して,uv^ixy^iz ∈ L (ii) |v| ≥ 1,かつ,|y| ≥ 1 (iii) |vxy| ≤ k. ただし,Dirichlet の原理は証明無しに使用してよい.",
...
2. CNF の性質（単規則やε規則の制限）および、パスの取り方（最長パスの最下部での繰り返し）から、v と y は空でない文字列とできる（※通常は |vy| > 0 だが、適切な文法と分割を選べば |v|≥1 かつ |y|≥1 を満たす構成が存在するケースを示すか、あるいは設問の条件に従いその存在を主張する）。
3. 重複を見つけた範囲はパスの長さ m+1 以内なので、その部分木が生成する文字列 vxy の長さは 2^{m+1} = k 以下である (iii condition)。`,
    commonMistakes: "条件 (ii) において、一般の Pumping Lemma では「|v|≥1 または |y|≥1 (|vy|>0)」ですが、本問では「かつ」となっています。多くの典型的な CFL (a^n b^n 等) ではこの強い条件も満たせますが、厳密には言語によります。証明の骨子は「パス上の記号の重複」です。"
  },
  {
    id: "5.3",
