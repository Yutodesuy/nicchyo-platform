...
export const problems: Problem[] = [
  // Section 1: Basic Mathematics (Sets, Functions, Graphs, Proofs)
  {
    id: "1.1",
    question: "集合 A, B について、以下の等式が成り立つことを示せ: (1) (A ∪ B)^c = A^c ∩ B^c, (2) (A ∩ B)^c = A^c ∪ B^c (De Morgan's Laws)",
    answer: "要素が属するかどうかで両辺が一致することを示す。",
    explanation: "ある要素 x が左辺 (AまたはB、ではない) に属するということは、x は A にも B にも属さないということと同じです。",
    formalProof: `(1) 任意の x について、
x ∈ (A ∪ B)^c
⇔ x ∉ A ∪ B
⇔ x ∉ A かつ x ∉ B
⇔ x ∈ A^c かつ x ∈ B^c
...
⇔ x ∈ A^c または x ∈ B^c
⇔ x ∈ A^c ∪ B^c`,
    commonMistakes: "「かつ」と「または」の否定を混同しないようにしましょう。ド・モルガンの法則は集合だけでなく論理式でも重要です。"
  },
  {
    id: "1.2",
    question: "集合 A, B, C について、A ∩ C = B ∩ C かつ A ∪ C = B ∪ C ならば A = B であることを示せ。",
    answer: "A の要素が B に含まれ、逆も成り立つことを示す。",
    explanation: "A の要素 x が C に入っている場合と入っていない場合で場合分けすると、どちらの場合も x は B に入らざるを得ないことがわかります。",
    formalProof: `任意の x ∈ A をとる。
Case 1: x ∈ C の場合
x ∈ A かつ x ∈ C なので、x ∈ A ∩ C。
仮定より A ∩ C = B ∩ C なので、x ∈ B ∩ C。
よって x ∈ B。
Case 2: x ∉ C の場合
x ∈ A なので x ∈ A ∪ C。
仮定より A ∪ C = B ∪ C なので、x ∈ B ∪ C。
つまり x ∈ B または x ∈ C だが、x ∉ C なので x ∈ B でなければならない。
いずれの場合も x ∈ B なので A ⊆ B。
同様に B ⊆ A も示せるため、A = B である。`,
    commonMistakes: "図（ベン図）を描いて納得するだけでは証明になりません。要素を用いた論理的な記述が必要です。"
  },
  {
    id: "1.3",
    question: "(1) f: R → R, f(x) = 2x は全単射か？ (2) f: Z → Z, f(x) = 2x は全単射か？",
    answer: "(1) はい (2) いいえ（単射だが全射でない）。",
    explanation: "実数ならどんな値も2で割れますが、整数の中で奇数は2で割れません（2xの形になりません）。",
    formalProof: `(1) R → R
単射性: f(x) = f(y) ⇒ 2x = 2y ⇒ x = y。OK。
全射性: 任意の y ∈ R に対し、x = y/2 とすれば f(x) = y となる。OK。
よって全単射。
(2) Z → Z
単射性: 同様に成立。
全射性: y = 1 (奇数) を考える。2x = 1 となる整数 x は存在しない。よって全射ではない。`,
    commonMistakes: "定義域と終域（実数か整数か）によって関数の性質が変わることに注意してください。"
  },
  {
    id: "1.6",
    question: "鳩の巣原理: f: [N+1] → [N] に対して、f(x) = f(y) となる異なる x, y が必ず存在することを示せ。",
    answer: "背理法または数学的帰納法で示す。",
    explanation: "N個の部屋にN+1人が入るなら、少なくとも1つの部屋には2人以上入ることになります。",
    formalProof: `背理法で示す。
すべての x, y (x ≠ y) に対して f(x) ≠ f(y) であると仮定する（単射であるとする）。
値域 f([N+1]) の要素数は、定義域と同じ N+1 個になるはずである。
しかし、終域は [N] = {1, ..., N} であり、要素数は N 個しかない。
集合の包含関係より |f([N+1])| ≤ |[N]| = N となるが、これは N+1 ≤ N を意味し矛盾。
よって仮定は誤りであり、f(x) = f(y) となる異なる x, y が存在する。`,
    commonMistakes: "「当たり前」で済なさず、集合の要素数（濃度）の矛盾として形式的に記述できるようにしましょう。"
  },
  {
    id: "1.8",
    question: "次のような次数の無向グラフは存在するか？ (1) 4頂点、次数 3,3,2,2 (2) 5頂点、次数 3,3,3,2,2",
    answer: "(1) 存在する (2) 存在しない。",
    explanation: "(1) 四角形に対角線を1本引けば作れます。(2) 次数の総和は必ず偶数（辺の数の2倍）になりますが、3+3+3+2+2=13 なので不可能です。",
    formalProof: `握手補題（Handshaking Lemma）より、グラフの次数の総和は辺の数の2倍（偶数）である。
(1) 和 = 3+3+2+2 = 10 (偶数)。構成可能（例: 頂点を {a,b,c,d} とし、辺 {a,b}, {b,c}, {c,d}, {d,a}, {a,c} を張ると、次数は a:3, b:2, c:3, d:2）。
(2) 和 = 3+3+3+2+2 = 13 (奇数)。
次数の和が奇数になるグラフは存在しないため、構成不可能。`,
    commonMistakes: "次数の和が偶数なら必ず存在すると勘違いしないでください（例: 次数 4,0,0,0,0 のような極端な例など、単純グラフでは作れない場合もあります）。"
  },
  {
    id: "1.14",
    question: "任意の無向グラフにおいて、奇数次数の頂点の数は偶数個であることを示せ。",
    answer: "握手補題からの帰結。",
    explanation: "次数の総和は偶数です。もし奇数次数の頂点が奇数個あれば、総和が奇数になってしまい矛盾します。",
    formalProof: `V を頂点集合とし、V_odd ⊆ V を奇数次数の頂点集合、V_even ⊆ V を偶数次数の頂点集合とする。
Σ_{v∈V} deg(v) = 2|E| （偶数）。
また、
Σ_{v∈V} deg(v) = Σ_{v∈V_odd} deg(v) + Σ_{v∈V_even} deg(v)。
ここで Σ_{v∈V_even} deg(v) は偶数の和なので偶数である。
よって Σ_{v∈V_odd} deg(v) も偶数でなければならない。
奇数を奇数回足すと奇数になるため、和が偶数になるには項数 |V_odd| が偶数でなければならない。`,
    commonMistakes: "証明問題として頻出です。直感だけでなく式変形として書けるようにしましょう。"
  },
  {
    id: "1.16",
    question: "連結グラフ G がオイラー閉路（全ての辺を一度だけ通って戻る）を持つための必要十分条件は、全ての頂点の次数が偶数であることである。これを示せ。",
    answer: "一筆書きの定理。",
    explanation: "ある頂点に入るたびに出る必要があるため、出入りのセットで次数は2ずつ増えます。",
    formalProof: `(必要性) オイラー閉路 C が存在するとする。
任意の頂点 v について、C が v を通過するたびに、入る辺と出る辺の2本が使われる。
始点兼終点であっても、最初に出る1本と最後に入る1本が対応する。
よって各頂点の次数は通過回数 × 2 となり、すべて偶数である。

(十分性) 全頂点が偶数次数かつ連結であるとする。
最長の単純路ではなく、辺を重複なく辿れるだけ辿る路 W を作る。
偶数次数条件より、W は必ず始点で終わる閉路になる（始点以外で止まると次数が奇数消費になるため矛盾）。
もし未使用の辺があれば、連結性より W 上の頂点と接続している。その頂点から新たな閉路 W' を作り W に結合する操作を繰り返すことで、全辺を含むオイラー閉路を構成できる（Hierholzerのアルゴリズム）。`,
    commonMistakes: "十分性の証明（構成）が難しい部分です。「行き止まりにならない」ことと「閉路をつなぎ合わせる」ロジックが必要です。"
  },
  {
    id: "1.22",
    question: "真理表を用いて (P ⇒ Q) ⇔ (¬Q ⇒ ¬P) （対偶）を示せ。",
    answer: "すべての真偽パターンで一致することを確認する。",
    explanation: "PならばQ、というのは、QでないならPではない（PならQなはずだから）ということと同じです。",
    formalProof: `真理表を作成する:
P | Q | P⇒Q | ¬Q | ¬P | ¬Q⇒¬P
--+---+-----+----+----+-------
T | T |  T  | F  | F  |   T
T | F |  F  | T  | F  |   F
...
(P⇒Q) と (¬Q⇒¬P) の列が完全に一致している。
よって論理的に等価である。`,
    commonMistakes: "含意 (⇒) は「前提が偽なら結論に関わらず真」になる点（Vacuous Truth）に注意してください。"
  },
  {
    id: "1.23",
    question: "「2=1」の証明の誤りを指摘せよ: a=b → a^2=ab → a^2-b^2=ab-b^2 → (a+b)(a-b)=b(a-b) → a+b=b → 2b=b → 2=1。",
    answer: "a-b で割り算している箇所が誤り。",
    explanation: "a=b なので a-b=0 です。0 で割ることは数学的に許されていません。",
    formalProof: `誤りは「両辺を (a-b) で割る」ステップにある。
前提として a = b としているため、a - b = 0 である。
等式の両辺を 0 で割る操作（除算）は定義されていない（あるいは、0 を掛けて等式を作ることはできても、戻すことはできない）。
したがって、(a+b)(a-b) = b(a-b) から a+b = b を導く部分は論理的に誤りである。`,
    commonMistakes: "式変形自体は正しく見えても、適用条件（分母が0でないこと）を確認する癖をつけましょう。"
  },
  {
    id: "1.25",
    question: "p を素数とするとき、√p が無理数であることを示せ。",
    answer: "背理法。有理数と仮定して矛盾を導く。",
    explanation: "既約分数 a/b で表せると仮定すると、a も b も p で割り切れることになり、既約であること（互いに素）に矛盾します。",
    formalProof: `背理法。√p が有理数であると仮定する。
√p = a/b （a, b は互いに素な自然数）と置ける。
両辺を二乗して分母を払うと、p b^2 = a^2。
左辺は p の倍数なので、a^2 は p の倍数。p は素数なので、a も p の倍数である (a = pk と置ける)。
代入すると、p b^2 = (pk)^2 = p^2 k^2。
p で割ると、b^2 = p k^2。
同様に b^2 は p の倍数なので、b も p の倍数である。
すると a と b は共に p の倍数となり、公約数 p を持つ。
これは a, b が互いに素であるという仮定に矛盾する。
よって √p は無理数である。`,
    commonMistakes: "「a^2 が p の倍数なら a も p の倍数」という事実は、p が素数でないと成り立ちません（例: 36 は 4 の倍数だが 6 は 4 の倍数でない）。素数の性質（ユークリッドの補題）を使っています。"
  },

  // Section 2: Languages and Automata
  {
    id: "2.1",
    question: "Σ={0,1}。以下の言語を認識する DFA を与えよ: (1) {w : w は 0 で始まり長さ偶数、または 1 で始まり長さ奇数}",
    answer: "状態遷移図を構成する。",
    explanation: "最初の文字を覚える状態と、長さの偶奇（パリティ）を覚える状態を組み合わせます。",
    formalProof: `状態集合 Q = {S, 0E, 0O, 1E, 1O, Dead}
- S: 開始
- 0E: 0開始・偶数長 (accept)
- 0O: 0開始・奇数長
- 1E: 1開始・偶数長
...
(Dead状態は不要、上記で完全定義可能)
受理状態 F = {0E, 1O}`,
    commonMistakes: "「または」条件の場合、並列に考えるか、条件を網羅する状態空間を作るかが鍵です。"
  },
  {
    id: "2.3",
    question: "正規言語は、連結操作 (Concatenation) について閉じていることを示せ。",
    answer: "NFA を連結する。",
    explanation: "Aの受理状態からBの開始状態へε遷移（空動作）で移動できるようにすれば、Aの後にBが続く文字列を認識できます。",
    formalProof: `言語 A, B を認識する NFA を N_A = (Q_A, Σ, δ_A, q_{A0}, F_A), N_B = (Q_B, Σ, δ_B, q_{B0}, F_B) とする。
一般性を失わず Q_A ∩ Q_B = ∅ とする。
連結言語 A∘B を認識する NFA N = (Q, Σ, δ, q_0, F) を以下のように構成する。
1. Q = Q_A ∪ Q_B
2. q_0 = q_{A0}
3. F = F_B
4. 遷移関数 δ: Q × Σ_ε → P(Q) を以下で定義する:
   任意の q ∈ Q, a ∈ Σ_ε に対して、
   - q ∈ Q_A の場合:
       δ(q, a) = δ_A(q, a)                 (もし q ∉ F_A または a ≠ ε)
       δ(q, a) = δ_A(q, a) ∪ {q_{B0}}      (もし q ∈ F_A かつ a = ε)
   - q ∈ Q_B の場合:
       δ(q, a) = δ_B(q, a)

この構成により、N は A の文字列を読み終わった直後（F_A に到達した時点）に、非決定的に ε 遷移で N_B の開始状態へ移動できる。
したがって L(N) = L(N_A) ∘ L(N_B) = A ∘ B となり、正規言語である。`,
    commonMistakes: "DFA 同士を連結しようとすると、「どこで A が終わって B が始まるか」がわからず困難です。NFA の非決定性（推測能力）を使うのが定石です。"
  },
  {
    id: "2.4",
    question: "C言語のコメント /* ... */ を認識する DFA を設計せよ（ネストなし）。",
    answer: "状態遷移で /* と */ を検出する。",
    explanation: "/ が来たら * を待ち、コメント中に入ったら * が来るのを待ち、その直後に / が来れば終了です。",
    formalProof: `Σ = {/, *, other}
状態:
0: 初期
1: / を読んだ（開始候補）
...
3 --/--> 4 (受理)
3 --*--> 3 (まだ終了候補)
3 --other--> 2 (終了失敗、コメント継続)
（受理状態 4 からは入力を受け付けないか、言語定義による。単一コメントの抽出ならここで停止）`,
    commonMistakes: "コメント終了の `*/` を検出する際、`**` のような並びや、`*` の後に `/` 以外が来た場合の「巻き戻し」処理（状態2に戻る）を正確に記述する必要があります。"
  },
  {
    id: "2.7",
    question: "ポンピング補題を用いて A = {0^n 1^n 2^n : n ≥ 0} が正規言語でないことを示せ。",
    answer: "文字列の一部を繰り返すとバランスが崩れる。",
    explanation: "正規言語なら、ある長さ以上の文字列には「繰り返しても言語に含まれる」部分(v)があるはずですが、0,1,2の個数が等しいという条件を守りながら繰り返すことができません。",
    formalProof: `背理法。A が正規であるとし、ポンピング長を p とする。
文字列 s = 0^p 1^p 2^p ∈ A を選ぶ (|s| = 3p ≥ p)。
ポンピング補題より、s = xyz と分割でき、|xy| ≤ p, |y| > 0, xy^i z ∈ A (∀i) が成り立つ。
条件 |xy| ≤ p より、y は s の先頭 p 文字以内にあるため、y は '0' のみからなる文字列である。
よって y = 0^k (k ≥ 1) と置ける。
ここで i=2 としてポンプアップ（yを2回繰り返す）すると、
s' = xy^2z = 0^{p+k} 1^p 2^p
となる。
A の定義より、A に属する文字列は 0, 1, 2 の個数がすべて等しくなければならない。
しかし s' においては、#0(s') = p+k > p = #1(s') = #2(s') となり、個数のバランスが崩れている。
よって s' ∉ A。
これは「任意の i ≥ 0 について xy^i z ∈ A」というポンピング補題の結論に矛盾する。
したがって A は正規言語ではない。`,
    commonMistakes: "「どんな分割に対しても矛盾を示す」のではなく、「ある分割が存在すると主張するポンピング補題に対して、その条件を満たせない（矛盾する）ような反例（iの選び方）を示す」という論理構造を理解しましょう。"
  },
  {
    id: "2.10",
    question: "加算等式 Add = {x=y+z : x,y,z は2進数} は正規言語か？",
    answer: "正規言語ではない。",
    explanation: "桁上がりや桁数のバランスを確認する必要があり、有限のメモリ（DFA）では足りません。",
    formalProof: `（標準表記: 入力は "x=y+z" の形で、x,y,z は先頭に 0 を持たない2進数 (0 のみ例外) とする。）
背理法で示す。Add が正規言語であると仮定し、ポンピング長を p とする。
次の文字列を取る:
s = 10^p = 1 + 1^p。
このとき左辺 10^p は 2^p、右辺 1 + 1^p は 1 + (2^p - 1) = 2^p なので、s ∈ Add。

ポンピング補題により s = xyz と分解でき、|xy| ≤ p, |y| > 0 かつ xy^i z ∈ Add (∀i ≥ 0) が成り立つ。
|xy| ≤ p より、y は左辺の先頭から p 文字以内にあり、"1 0^{p-1}" の部分だけからなる。
よって y = 0^k (k ≥ 1) か、あるいは "1 0^k" の部分文字列になるが、いずれにせよ左辺の桁数だけが変化する。

i = 0 を考える（ポンプダウン）。
すると左辺は 10^{p-k} もしくは 10^{p-k-1} の形になり、その値は 2^{p-k}（または 2^{p-k-1}）。
一方、右辺 "1 + 1^p" は 2^p のままで変化しない。
したがって 2^{p-k} ≠ 2^p となり、xy^0 z ∉ Add。

これは「∀i ≥ 0 で xy^i z ∈ Add」という結論に矛盾する。
よって Add は正規言語ではない。`,
    commonMistakes: "「足し算は簡単だからできる」と思いがちですが、有限オートマトンは「入力全体を覚えておけない」ため、桁数が任意に大きくなると対応できません。"
  },
  {
    id: "2.12",
    question: "文法 E → E+T | T, T → T×F | F, F → (E) | a における 'a+a×a' の構文解析木を示せ。",
    answer: "演算子の優先順位に従って木を作る。",
    explanation: "×の方が結合力が強いため、a+a×a は a+(a×a) として解析されます。",
    formalProof: `導出過程:
E ⇒ E + T
  ⇒ T + T
  ⇒ F + T
...
構文解析木:
    ...
    | |
    a a
これにより、掛け算が足し算より深い（先に結合する）構造となる。`,
    commonMistakes: "E → E+E のような曖昧な文法だと解析木が複数できてしまいますが、この文法は演算子の優先順位が一意に決まるように設計されています。"
  },
  {
    id: "2.13",
    question: "Σ={0,1}。{w : w は同じ文字で始まり終わる} を生成する CFG を与えよ。",
    answer: "S → 0A0 | 1A1 | 0 | 1, A → ...",
    explanation: "最初と最後を固定し、真ん中は何でも良い (0|1)* とします。",
    formalProof: `S → 0 M 0 | 1 M 1 | 0 | 1
M → 0 M | 1 M | ε

解説:
- 0 M 0: 0で始まり0で終わる（長さ2以上）
- 1 M 1: 1で始まり1で終わる（長さ2以上）
- 0, 1: 長さ1の場合
- M: 任意の文字列（中間部分）を生成
これにより、最初と最後の文字が一致する任意の文字列を生成できる。`,
    commonMistakes: "長さ1の文字列（'0', '1'）も「同じ文字で始まり終わる」に含まれるのを忘れがちです。"
  },
  {
    id: "2.17",
    question: "言語 {ww : w ∈ {0,1}*} は文脈自由言語(CFL)でないことを示せ。",
    answer: "CFL のポンピング補題を用いる。",
    explanation: "文字列の前半と後半で全く同じコピーを持つ構造は、スタック（LIFO）ではチェックできません（FIFO的な構造が必要）。",
    formalProof: `CFL のポンピング補題を適用する。
s = 0^p 1^p 0^p 1^p ∈ A を選ぶ。
s = uvxyz, |vxy| ≤ p, |vy| > 0 として分解する。
vxy は s の部分文字列（長さ p 以下）なので、s の「境界」をまたいで多くの異なる文字種を含むことはできない。
1. v, y が前半の 0^p 1^p 内にある場合: ポンプアップすると前半だけ長くなり、後半と一致しなくなる。
2. v, y が後半内にある場合: 同様に不一致。
3. 中央の境界をまたぐ場合: 前半の '1' と後半の '0' の数が変わるが、前半の '0' や後半の '1' は変わらないため、形が崩れる (0^p 1^q 0^r 1^p のようになり、前半≠後半)。
いずれの場合も ww の形を保てないため、CFL ではない。`,
    commonMistakes: "「回文 {w w^R} は CFL だから、コピー {ww} も CFL だろう」というのは直感的な誤りです。スタックは逆順の照合には向いていますが、正順の照合には向いていません。"
  },
  {
    id: "2.20",
    question: "言語 {x]y : x ≠ y} は文脈自由言語であることを示せ。",
    answer: "非決定的に「異なる位置」を推測する。",
    explanation: "xとyが違うということは、あるk文字目が違うか、長さが違うかです。PDAはその「違い」を非決定的に探します。",
    formalProof: `以下の和集合と考える:
1. |x| ≠ |y| の場合:
   - xの方が長い、あるいはyの方が長い。これは数えるだけなのでCFL。
2. |x| = |y| かつ、ある位置 k で x[k] ≠ y[k] の場合:
   - PDA は入力を読みながら、非決定的に「ここが違うはずの位置 k だ」と推測する。
   - k に到達するまでは単に文字をスタックに積んで数を数える（あるいは無視）。
   - k 文字目の記号を記憶する。
   - 区切り文字 ']' を通過。
   - y の k 文字目までカウンタ（スタック）を使って進む。
   - x[k] と y[k] が異なることを確認する。
   - 残りの文字を読み飛ばして accept。
これは非決定性 PDA で構成可能なので CFL である。`,
    commonMistakes: "「等しくない」ことの証明は、「等しい」ことの確認（全文字一致）よりも簡単な場合があります（1箇所でも違えばいいから）。"
  },
  {
    id: "2.21",
    question: "決定性文脈自由言語 (DCFL) は、(i)和集合、(ii)共通部分 などの演算について閉じているか？",
    answer: "閉じていない。",
    explanation: "DCFL（決定性PDAで受理可能）は補集合については閉じていますが、和集合や共通部分については閉じていません。",
    formalProof: `(i) 和集合について閉じていない反例:
L1 = {a^i b^j c^k : i ≠ j} (DCFL)
L2 = {a^i b^j c^k : j ≠ k} (DCFL)
L1 ∪ L2 の補集合は {a^i b^j c^k : i=j かつ j=k} = {a^n b^n c^n} となる。
これは CFL ですらない。
もし DCFL が和集合で閉じているなら、補集合でも閉じているため、{a^n b^n c^n} も DCFL (したがって CFL) となり矛盾する。
(ii) 共通部分も同様に閉じていない（CFL 自体が閉じていない）。`,
    commonMistakes: "正規言語の閉包性質（全部OK）や、一般の CFL の閉包性質（和はOK、共通部分はNG）と混同しやすいので整理が必要です。"
  },
...
M' はテープを「(アドレス, 値) のペアを記録するログ」として使用する。
初期テープ: (空白)
動作シミュレーション (ステップ t):
1. 読み取り (Read):
   M が論理的なテープ位置 i を読もうとするとき、M' は自身のテープの右端（最新）から左へ向かって走査する。
   最初に遭遇したペア (addr, val) が addr = i であれば、その val を読み取り値とする。
   テープの左端まで走査しても見つからない場合、初期値（空白）とする。
2. 書き込み (Write):
   M が位置 i に値 v を書き込もうとするとき、M' はテープの未使用領域（右端）に新しいペア (i, v) を書き込む。
   （以前に (i, old_v) が存在していても、上書きせず追記するだけなので、各物理セルへの書き込みは1回のみという制約を満たす。）

このように、論理的な書き換え操作を物理的な追記操作に変換することで、M' は M の動作を完全にシミュレートできる。
（計算時間は各ステップでログ長に比例して増加するが、計算可能性には影響しない。）`,
...
写像帰着の定義より、計算可能関数 f が存在して、
  w ∈ A_TM ⇔ f(w) ∈ E_TM
が成り立つ。

このとき補集合にも同じ f が使えることに注意する:
  w ∈ A_TM^c ⇔ w ∉ A_TM ⇔ f(w) ∉ E_TM ⇔ f(w) ∈ E_TM^c
よって A_TM^c ≤m E_TM^c が成立する。

ここでクラスの性質を使う。
E_TM^c = { <M> : L(M) ≠ ∅ } は RE である:
各 <M> に対して、すべての入力文字列を並列シミュレーションし、
M がどれか1つを受理したら accept すれば良い（受理があれば有限時間で見つかる）。

したがって、A_TM^c ≤m E_TM^c かつ E_TM^c ∈ RE なら、
RE への写像帰着の閉包性より A_TM^c ∈ RE となる。
しかし A_TM^c は RE ではない（A_TM が決定不能であるため A_TM は co-RE でない）。

矛盾が生じたので、A_TM ≤m E_TM は成り立たない。`,
...
これは n が大きければ 2^n に近く、ほとんどの文字列は圧縮不可能であることを示す。`,
  ...
  },
  {
    ...
    formalProof: `背理法。K(x) が計算可能関数であると仮定する。
以下の TM M を構成する。
M(n):
  1. すべての文字列 x を辞書順に列挙する。
  2. 各 x について K(x) を計算する（仮定より可能）。
  3. K(x) > n となる最初の x を見つけたら、その x を出力して停止する。

この機械 M は固定された定数サイズのプログラム記述 <M> を持つ（長さ C とする）。
M(n) を実行するために必要な記述は、<M> と入力 n である。
よって、M(n) が出力する文字列 x の記述複雑性 K(x) は、以下で抑えられる：
K(x) ≤ |<M>| + |n| + c' = C + log n + c' （c' はオーバーヘッド定数）。

一方、M(n) の定義より、出力される x は K(x) > n を満たす。
したがって、C + log n + c' ≥ K(x) > n。
すなわち log n + (定数) > n となる。
しかし、十分大きな n に対して n > log n + (定数) であるため、これは矛盾である。
よって K(x) は計算不可能である。`,
...
従って同時にすべての節を満たす割り当ては存在せず、式は充足不可能である。`,
...
2. 任意の A ∈ NP に対して A ≤p SAT を示す (Cook-Levin の定理)。
A を判定する非決定性 TM N が入力 w に対して O(n^k) 時間で走るとする。
N の計算過程（Tableau）を (n^k) × (n^k) のグリッドで表す。
ブール変数 X_{i,j,s} を「ステップ i, 位置 j のセルに記号 s がある (またはヘッドがあり状態 s である)」とする。
論理式 φ = φ_cell ∧ φ_start ∧ φ_accept ∧ φ_move を構成する:
- φ_cell: 各セル (i,j) に正確に1つの記号が入る (X_{i,j,s} の排他的論理和)。
- φ_start: 第0ステップ (i=0) の内容が初期構成 (q0, w) と一致する。
- φ_accept: 最終ステップまでのいずれかのセルに受理状態 q_accept が現れる。
- φ_move: 任意の (i,j) について、(i,j) の値が (i-1) ステップ目の近傍 (j-1, j, j+1) の値と遷移関数 δ に矛盾しない（2×3 ウィンドウの整合性）。

これらの条件式はすべて局所的なチェックであり、全体のサイズは O(n^{2k}) (多項式) となる。
φ が充足可能 ⇔ N が w を受理する計算パスが存在する。
よって SAT は NP 完全である。`,
...
よって 2SAT は多項式時間で判定可能。`,
  ...
  },
  {
    ...
    formalProof: `1. VC ∈ NP:
頂点被覆集合 C が与えられたら、全ての辺 (u, v) に対して u ∈ C または v ∈ C を確認すればよい。
これは O(|E|) で検証可能なので VC ∈ NP。

2. 3SAT ≤p VC を示す。
3CNF φ = ∧_{j=1}^m (ℓ_{j1} ∨ ℓ_{j2} ∨ ℓ_{j3}) を取る。
以下のグラフ G を構成する。

【変数ガジェット】
各変数 x_i に対し 2 頂点 x_i, ¬x_i を置き、辺 (x_i, ¬x_i) を張る。

【節ガジェット】
各節 C_j に対し 3 頂点 ℓ_{j1}, ℓ_{j2}, ℓ_{j3} を三角形で結ぶ。

【接続】
節ガジェットの頂点 ℓ_{jk} と、対応する変数頂点 (x_i または ¬x_i) を辺で結ぶ。

被覆サイズを k = n + 2m とする。

（⇒）φ が充足可能だとする。
変数ごとに、真にするリテラルの頂点を選んで C に入れる（各変数ガジェットで1頂点）。
各節では、真のリテラルは少なくとも1つあるため、その頂点以外の2頂点を C に入れる。
すると変数ガジェットの辺は必ず被覆され、節の三角形も2頂点で被覆される。
また、節頂点と変数頂点の接続辺は、節側または変数側が選ばれているので被覆される。
合計サイズは n + 2m = k。

（⇐）G にサイズ k の頂点被覆 C が存在するとする。
各変数ガジェットの辺 (x_i, ¬x_i) を被覆するために、少なくとも1頂点を選ぶ必要がある。
サイズ制限 k = n + 2m より、各変数ガジェットでちょうど1頂点、各節ガジェットでちょうど2頂点が選ばれている。
節ガジェットで選ばれなかった1頂点 ℓ_{jk} は、三角形の残り2辺を被覆するために「真」と解釈できる。
この ℓ_{jk} に対応する変数リテラルを真に割り当てると、各節に少なくとも1つ真リテラルが存在する。
よって φ は充足可能。
...
候補パス (頂点の順列) が与えられたら、各頂点がちょうど1回出現し、隣接頂点が辺で結ばれているかを O(|V|+|E|) で検証できる。

2. 3SAT ≤p HamPath を示す（標準的なガジェット構成）。
3CNF φ = ∧_{j=1}^m (ℓ_{j1} ∨ ℓ_{j2} ∨ ℓ_{j3}) を取る。
各変数 x_i について「真を選ぶと上側、偽を選ぶと下側」を通る二分岐のパスガジェット G_i を作る。
このガジェットは入口から出口まで Hamilton 路で通過するには、上か下のいずれか一方の枝を選ばざるを得ない構造にする。

各節 C_j について節頂点 c_j を作る。
リテラル ℓ_{jk} が x_i（または ¬x_i）であるとき、G_i の上側（または下側）枝の適切な位置から c_j へ寄り道して戻れるように「横枝」を張る。
ただし、Hamilton 路が全頂点を1回だけ通るように、横枝は c_j を訪れた後は必ず元の枝に戻る構造にする。

（⇒）φ が充足可能だとする。
各変数に対して真偽を固定し、対応する枝（上/下）を通るように Hamilton 路を構成する。
節 C_j は少なくとも1つ真リテラルを含むので、対応する枝から c_j に寄り道して訪れることができる。
全ての節頂点を訪れつつ、全ガジェットを直列に通過できるため Hamilton 路が存在する。

（⇐）G に Hamilton 路が存在するとする。
各変数ガジェット G_i は、構造上、上枝か下枝のどちらか一方しか選べない。
選ばれた枝に対応して x_i を真/偽に割り当てる。
Hamilton 路はすべての節頂点 c_j を訪れる必要があるため、各 c_j は少なくとも1つの枝から到達可能でなければならない。
したがって各節 C_j には少なくとも1つ真リテラルが存在し、φ は充足可能。

以上より HamPath は NP 完全。`,
...
分割集合 (A, B) を証拠として与えられたら、和が等しいかを O(n) で検証できる。

2. SubsetSum ≤p Partition を示す。
SubsetSum の入力を S = {x_1, ..., x_n}, ターゲット t とする。
S の総和を m = Σ_i x_i とする。
以下の集合を構成する:
  S' = S ∪ {2m - t, m + t}。
S' の総和は m + (2m - t) + (m + t) = 4m。
Partition の目標は「総和 4m の半分 2m に分けられるか」である。

（⇒）S に部分集合 T ⊆ S が存在して Σ(T) = t だとする。
片方の分割として T ∪ {2m - t} を取ると、その和は
  Σ(T) + (2m - t) = t + 2m - t = 2m。
残りは (S \ T) ∪ {m + t} となり、こちらも
  (m - t) + (m + t) = 2m。
よって Partition は可能。

（⇐）S' が 2m と 2m に分割できるとする。
2m - t と m + t はどちらも m を超える大きな数なので、同じ側に入ると 2m を超える。
従って 2m - t と m + t は必ず別側に分かれる。
2m - t を含む側の残りは S の部分集合 T であり、
  (2m - t) + Σ(T) = 2m
を満たすため、Σ(T) = t。
よって SubsetSum の解が存在する。

以上より Partition は NP 完全。`,
...
彩色（各頂点の色割当）が与えられれば、各辺で色が異なるかを O(|E|) で検証できる。

2. 3SAT ≤p 3Color を示す。
3CNF φ = ∧_{j=1}^m (ℓ_{j1} ∨ ℓ_{j2} ∨ ℓ_{j3}) を取る。

【パレットガジェット】
3頂点 T, F, B を三角形で結ぶ。これにより 3 色が固定される。

【変数ガジェット】
各変数 x_i に対して頂点 x_i と ¬x_i を作り、辺 (x_i, ¬x_i) を張る。
さらに x_i と ¬x_i の両方を B に接続する。
すると x_i と ¬x_i は B 色を取れず、T または F のどちらか一方に着色される。

【節ガジェット】
各節 C_j = (ℓ_{j1} ∨ ℓ_{j2} ∨ ℓ_{j3}) に対して、標準的な OR ガジェットを接続する。
具体的には、新しい頂点 c_j を追加し、c_j を B に接続しつつ、
ℓ_{j1}, ℓ_{j2}, ℓ_{j3} と結ぶ補助構造を作り、
「3つのリテラルがすべて F 色である場合のみ彩色不能」になるように設計する。
（例えば K_4 から辺を1本除いた構造を用いる標準構成で実現できる。）

（⇒）φ が充足可能なら、変数割当で真を T、偽を F として彩色する。
各節には少なくとも1つ真リテラルがあるため、節ガジェットは彩色可能となり、全体が 3 彩色できる。

（⇐）全体が 3 彩色可能なら、変数ガジェットより各変数は T/F のどちらかに塗られる。
節ガジェットの性質により、節のリテラルがすべて F だと彩色不能になるので、
各節には少なくとも1つ T 色のリテラルが存在する。
よって対応する割当は φ を充足する。
...
以上より 3Color は NP 完全。`,
  ...
  },
  {
    ...
    formalProof: `任意の PSPACE 言語 L を判定する機械 M (空間 S(n)=n^k) を考える。
M の構成（Configuration）はテープ内容、状態、ヘッド位置を含み、サイズは O(n^k)。全構成数は 2^{O(n^k)}。
式 ψ(C1, C2, t) を「構成 C1 から C2 へ t ステップ以内で到達可能か」と定義する。
目標は ψ(C_start, C_accept, 2^{O(n^k)}) を作ること。
再帰的構成:
1. t = 1 のとき:
   C1 = C2 または C1 → C2 が1ステップで遷移可能。これは論理式で直接記述できる（サイズ O(n^k)）。
2. t > 1 のとき:
   ψ(C1, C2, t) ≡ ∃Cm ∀(x, y) ∈ {(C1, Cm), (Cm, C2)} [ ψ(x, y, t/2) ]
   これを論理式に展開すると:
   ∃Cm ∀x ∀y [ ((x=C1 ∧ y=Cm) ∨ (x=Cm ∧ y=C2)) ⇒ ψ(x, y, t/2) ]

この再帰の深さは log(2^{O(n^k)}) = O(n^k)。
各ステップで変数を追加するが、多項式個に収まる。
もし ∀ を使わず単純に ψ(C1,Cm,t/2) ∧ ψ(Cm,C2,t/2) と展開すると、式のサイズが倍々になり指数爆発するが、∀ による集約で多項式サイズに抑えられる。
よって TQBF は PSPACE 困難。`,
...
s-t パス以外の閉路がないグラフ（例: 層状グラフ）に変換してから (t, s) を足せばよい。`,
  ...
  {
    id: "5.1",
    question: "言語 A, B について,言語 A/B を次のように定める: A/B := {x : ある文字列 y ∈ B が存在して,xy ∈ A が成り立つ.} このとき,言語 A が正規言語であるならば,任意の言語 B に対して,A/B が正規言語であることを証明せよ.",
    answer: "A を受理する DFA の受理状態集合を拡張することで構成できる。",
    explanation: "正規言語 A を受理する DFA において、「そこから B の何らかの文字列を読み込むと受理状態に到達できる状態」の集合を考え、それを新しい受理状態とすれば、A/B を受理する DFA になります。",
    formalProof: `言語 A を受理する DFA を M = (Q, Σ, δ, q_0, F) とする。
言語 A/B を受理する DFA M' = (Q, Σ, δ, q_0, F') を以下のように構成する。
ここで、新しい受理状態集合 F' を次のように定義する:
F' = { q ∈ Q | ∃y ∈ B, δ(q, y) ∈ F }。
（すなわち、状態 q から出発して、B に属する何らかの文字列 y を読み込んだときに、元の受理状態 F に到達可能であれば、q を F' に含める。）
注: 集合 F' は、B が正規言語でなくても一意に定まる Q の部分集合（有限集合）であるため、M' は正当な DFA である。
...
⇔ ∃y ∈ B, xy ∈ L(M) = A
⇔ x ∈ A/B
したがって、M' は A/B を受理する。よって A/B は正規言語である。`,
    commonMistakes: "B が正規言語である必要はありません。「B の文字列で到達可能」という判定は、DFA の構成時には静的に（無限の時間をかけてでも）決定される集合の定義に使われるだけで、実行時の計算には含まれないからです。"
  },
  {
    id: "5.2",
    question: "任意の文脈自由言語 L に対して,下記の条件を満たす非負整数 k ∈ Z≥0 が存在することを証明せよ:長さが少なくとも k である任意の文字列 s ∈ L に対して,次の三つの条件を満たす 5 個の断片 s = uvxyz に分割できる: (i) 全ての非負整数 i ∈ Z≥0 に対して,uv^ixy^iz ∈ L (ii) |vy| ≥ 1 (すなわち |v| ≥ 1 または |y| ≥ 1) (iii) |vxy| ≤ k. ただし,Dirichlet の原理は証明無しに使用してよい.",
    answer: "文脈自由文法の Pumping Lemma (反復補題) の証明。",
    explanation: "文法を Chomsky 標準形に変換し、長い文字列の導出木（Parse Tree）におけるパスの長さが非終端記号の数を超えると、必ず同じ非終端記号が繰り返し現れる（Dirichlet の原理）ことを利用します。",
    formalProof: `L を生成する Chomsky 標準形 (CNF) の文法 G = (V, Σ, R, S) を考える。|V| = m とする。
k = 2^{m+1} とする。
|s| ≥ k である任意の文字列 s ∈ L を考える。
s の導出木において、根から葉までの最長パスの長さは少なくとも m+1 である（2分木で高さ h の葉の数は最大 2^h なので）。
この最長パス上には、少なくとも m+2 個の頂点（非終端記号）がある。
非終端記号の種類は m 個しかないので、Dirichlet の原理により、パスの下の方（葉に近い側）の m+1 個の頂点の中で、ある非終端記号 A が重複して現れる。
下側の A を A_lower、上側の A を A_upper とする。
A_upper から A_lower への導出、および A_lower からの導出により、文字列は s = uvxyz と分割される。
ここで A_upper ⇒ v A_lower y ⇒ vxy、A_lower ⇒ x となる。
1. A_upper は再帰的に現れるので、v と y を i 回繰り返した uv^ixy^iz も G で導出可能である (i condition)。
2. CNF には ε 規則や単規則が存在しない（S→εを除くが、長い文字列の導出には関係しない）ため、分岐が発生するたびに文字列の長さは増加する。A_upper から A_lower への導出過程で少なくとも1回は規則が適用されており、かつ非終端記号が分岐しているか、終端記号を生成しているため、v または y の少なくとも一方は空でない（|vy| ≥ 1）。
3. 重複を見つけた範囲はパスの長さ m+1 以内なので、その部分木が生成する文字列 vxy の長さは 2^{m+1} = k 以下である (iii condition)。`,
    commonMistakes: "条件 (ii) は「|v| ≥ 1 かつ |y| ≥ 1」ではなく、「|v| ≥ 1 または |y| ≥ 1」です。導出木の形状によっては片方が空になることがあります（例：右再帰のみの文法など）。しかし、CNF の性質上、両方が同時に空になることはありません（単規則のループがないため）。"
  },
  {
    id: "5.3",
    question: "以下の各設問に答えよ: (1) 言語 A_TM := {<M, w> : Turing 機械 M は入力 w を受理する.} が判定不可能であることをカントールの対角線論法を用いて証明せよ. (2) 2 テープ Turing 機械が任意の入力に対して,第 2 テープに非空白文字を書き込むかどうかを判定する問題を言語として定式化せよ. (3) 問 3(2) で定式化した言語は判定可能であるか? 判定不可能であるならば,その事実を言語 A_TM からの写像帰着を与えることで証明せよ.",
    answer: "(1) 対角線論法 (2) 言語定義 (3) 判定不可能。",
    explanation: "(1) もし判定可能なら、自分自身を入力として逆の動作をする機械を作って矛盾させます。(3) 「受理するときだけ第2テープに書く」機械を作れば、第2テープへの書き込み判定が受理判定と等価になります。",
    formalProof: `(1) A_TM が判定可能と仮定し、判定機 H が存在するとする。
新しい TM D を構成する:
D(<M>) = {
  reject, if H(<M, <M>>) accepts
  accept, if H(<M, <M>>) rejects
}
D(<D>) を考えると、
D accepts <D> ⇔ H(<D, <D>>) rejects ⇔ D does not accept <D> (定義より H は A_TM の判定機なので)。
これは矛盾。よって A_TM は判定不可能。

(2) 言語 L_Write2 を以下のように定める:
L_Write2 = { <M, w> : M は 2テープ TM であり、入力 w を与えたとき、計算の過程で第 2 テープに少なくとも 1 回、非空白文字を書き込む }。

(3) L_Write2 は判定不可能である。A_TM ≤m L_Write2 を示す。
A_TM への入力 <M, w> に対し、以下の機能を持つ 2テープ TM M' を構成する変換 f(<M, w>) = <M', w> を考える。
M' の動作:
1. 第 2 テープには何も書かずに、第 1 テープ上で M(w) のシミュレーションを行う。
2. もし M が w を受理 (accept) したら、第 2 テープに '1' を書き込み、停止（accept）する。
3. もし M が w を拒否 (reject) したら、第 2 テープには何も書かずに停止（reject）する。
解析:
- M が w を受理する場合: M' は第 2 テープに書き込む。よって <M', w> ∈ L_Write2。
- M が w を受理しない（reject または loop）場合: M' は第 2 テープに書き込まない。よって <M', w> ∉ L_Write2。
この変換 f は計算可能であり、A_TM の判定不能性が L_Write2 に遺伝する。`,
    commonMistakes: "帰着の方向を間違えないでください（未知の問題「が」既知の難問「以上に」難しいことを言うため、既知 ≤ 未知）。また、M' は M そのものではなく、M をラップして挙動を制御した新しい機械です。"
  },
  {
    id: "5.4",
    question: "無向グラフ G について,各々が辺で結ばれている相異なる 3 点を三角形と呼ぶ. 言語 Triangle Free := {<G> : G は,三角形を含まない単純無向グラフである.} が,頂点数の多項式時間で判定できることを証明せよ.",
    answer: "すべての3点の組を調べる (O(n^3))。",
    explanation: "頂点数 n に対して、3点の組み合わせは nC3 通りあり、これは O(n^3) です。各組み合わせについて辺があるか調べる処理は多項式時間で終わります。",
    formalProof: `Triangle Free を判定するアルゴリズム M を以下のように構成する。
入力: 単純無向グラフ G = (V, E)。頂点数を n とする。
1. V の中から、互いに異なる 3 つの頂点の組 {u, v, w} のすべてに対してループを実行する。
   （組み合わせの総数は binom{n}{3} = n(n-1)(n-2)/6 = O(n^3) である。）
2. 各組 {u, v, w} について、それらが互いに辺で結ばれているか、すなわち (u, v)∈E かつ (v, w)∈E かつ (w, u)∈E であるかを確認する。
   （隣接行列を用いれば判定は O(1)、隣接リストでも O(n) 以下で可能。）
3. もしそのような組（三角形）が見つかった場合、G は Triangle Free ではないので、reject する。
4. すべての組を調べ終わり、三角形が見つからなかった場合、accept する。

このアルゴリズムの計算時間は O(n^3) × O(1) = O(n^3) （隣接行列の場合）であり、入力サイズ（頂点数 n や辺数 m）に対する多項式時間である。
よって Triangle Free ∈ P である。`,
    commonMistakes: "「判定できることを証明せよ」という問題は、具体的なアルゴリズムを提示し、その計算量が多項式オーダーであることを示せば十分です。隣接行列の3乗 (A^3) の対角成分を見る方法 (O(n^2.37...)) もありますが、単純な全探索で十分正解です。"
  }
];
