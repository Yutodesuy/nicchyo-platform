...
受理状態 F = {0E, 1O}`,
  ...
  },
  {
    ...
    formalProof: `言語 A, B を認識する NFA を N_A, N_B とする。
N_A の各受理状態から、N_B の開始状態へ ε遷移を追加する。
新しい開始状態は N_A の開始状態。
新しい受理状態は N_B の受理状態のみ（元の N_A の受理状態は非受理に変更）。
このように構成した NFA N は、A の文字列を読み終えた後、非決定的に B の処理に移行し、B として受理される場合に全体を受理する。
よって L(N) = A ∘ B となり、正規言語となる。`,
...
|xy| ≤ p なので、y は s の先頭 p 文字以内、つまり '0' の部分だけに含まれる。
よって y = 0^k (k ≥ 1) である。
ここで i=2 としてポンプアップすると、
xy^2z = 0^{p+k} 1^p 2^p
となる。
0の数は p+k、1と2の数は p なので、各文字数が等しくない。
よって xy^2z ∉ A。これは矛盾である。
したがって A は正規言語ではない。`,
