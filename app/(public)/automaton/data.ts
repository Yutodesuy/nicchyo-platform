...
したがって A は正規言語ではない。`,
  ...
  },
  {
    ...
    formalProof: `(厳密には入力形式によるが、一般的な文字列として)
ポンピング補題を用いる。
s = 1^p = 0 + 1^p （のような式を考える、あるいは単純に長さの制約を利用）。
もし x, y, z の順序が固定されている場合、桁数の整合性をチェックするには、離れた場所にあるビット同士の相関を見る必要がある。
例えば x の長さを変えずに y だけポンプアップ（桁を増やす）すると等式が成立しなくなる。
(より簡単な証明: Add ∩ {1^n = 0 + 1^m} のような共通部分を取ると {1^n = 0 + 1^n} ≅ {0^n 1^n} の構造が見え、非正規性が示せる)`,
