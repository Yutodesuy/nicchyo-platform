      ...
      <div className="min-h-screen bg-[#FFFAF0] pb-24">
        <div className="mx-auto max-w-2xl px-4 pt-16 text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">
            {formatExpirationLabel(expirationPreset, customDateTime)}
            の間、マップ上に表示されます
          </p>
      ...
      <form onSubmit={handleSubmit} className="mx-auto max-w-2xl space-y-4 px-4 pt-5">
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="flex items-center justify-between px-4 pb-3">
            <span className={`text-xs ${charCount > MAX_CHARS * 0.9 ? "text-red-500" : "text-slate-400"}`}>
              {charCount} / {MAX_CHARS}
            </span>
            {charCount === 0 && (
            ...
            {EXPIRATION_OPTIONS.map((opt) => {
              ...
              return (
                <button
                  key={opt.preset}
                  type="button"
                  onClick={() => setExpirationPreset(opt.preset)}
                  className={`flex flex-col items-center gap-1.5 rounded-xl border-2 py-3 px-2 text-center transition ${
                    isSelected
            ...
            {showPreview && (
              <div className="rounded-2xl border border-amber-100 bg-white p-4 shadow-sm">
                <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-slate-800">山田農園</p>
                ...
                <p className="mt-3 text-sm text-slate-800 whitespace-pre-wrap">{text}</p>
                {imagePreview && (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img
                    src={imagePreview}
